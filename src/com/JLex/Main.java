 ...
 * allocation. 7) Reading and writing in Unicode. Use DataInputStream and
 * DataOutputStream, along with writeUTF??? 8) Fix or at least annotate ^x bug.
 * 9) Add to spec about extending character set. 10) making sure newlines are
 * handled correctly S and consistly with the m_unix flag 11) m_verbose -- what
 * should be done with it? 12) turn lexical analyzer into a coherent Java
...
class CEmit {
     ...
     *
     * if (CUtility.DEBUG) { CUtility.cuAssert(null != m_spec);
    ...
    private void emit_construct() throws java.io.IOException {
        ...
        if (m_spec.m_integer_type || true == m_spec.m_yyeof) {
            m_outstream.writeBytes("\t/** Returned by yylex() if we\n");
            m_outstream.writeBytes("\t *  are at the end of file.\n");
        ...
        m_outstream.writeBytes("\t/** Create a Lexer that reads from\n");
        m_outstream.writeBytes("\t *  a Reader.\n");
        m_outstream.writeBytes("\t *  @param reader The Reader.\n");
        m_outstream.writeBytes("\t */\n");
        m_outstream.writeBytes("\t");
        ...
        m_outstream.writeBytes("\t/** Create a Lexer that reads from\n");
        m_outstream.writeBytes("\t *  an InputStream.\n");
        m_outstream.writeBytes("\t *  @param instream The InputStream.\n");
        m_outstream.writeBytes("\t */\n");
    ...
    private void emit_helpers() throws java.io.IOException {
         ...
         * {\n"); m_outstream.writeBytes("\t\treturn
         * yy_state_dtrans[state];\n"); m_outstream.writeBytes("\t}\n");
         */
        ...
        /* Function: yy_next */
        /*
         * m_outstream.writeBytes("\tprivate int yy_next (int current,char
         * lookahead) {\n"); m_outstream.writeBytes("\t\treturn
    ...
    private void emit_table() throws java.io.IOException {
        ...
        /*
         * yy_nxt[][] is assigned result of "int [][] unpackFromString(int
         * size1, int size2, String st)". yy_nxt[][] values are coded into a
         * string by printing integers and representing integer sequences as
         * "value:length" pairs.
         */
    ...
    private void emit_driver() throws java.io.IOException {
        ...
        m_outstream.writeBytes("\t/** Perform lexical analysis.\n");
        m_outstream.writeBytes("\t *  @return The token.\n");
        m_outstream
                .writeBytes("\t *  @exception java.io.IOException If there\n");
        m_outstream.writeBytes("\t *  are problems reading data.\n");
        m_outstream.writeBytes("\t */\n");
        ...
        /*
         * m_outstream.writeBytes("\t\tyy_this_accept =
         * yy_accept(yy_state);\n");
         */
        ...
        /*
         * m_outstream.writeBytes("\t\t\t\t\tyy_last_accept_state =
         * YY_NO_STATE;\n");
         * m_outstream.writeBytes("\t\t\t\t\tyy_mark_start();\n");
         */
        /*
            ...
            if (false == (CLexGen.ANY == m_spec.m_current_token
                ...
                /* Match dot (.) using character class. */
                    ...
                    if (CLexGen.AT_BOL == m_spec.m_current_token) {
                        ...
                        /*
                         * start.m_set.add((byte) '\n'); if (false ==
                         * m_spec.m_unix) { start.m_set.add((byte) '\r'); }
                         */
                        start.m_set.complement();
                    }
        ...
        for (index = 0; index < size; ++index) {
            ...
            if (b == state.m_edge
                ...
                if (null == bunch.m_nfa_set) {
                    ...
                    /*
                     * bunch.m_nfa_bit = new
                     * JavaLexBitSet(m_spec.m_nfa_states.size());
                     */
                    bunch.m_nfa_bit = new JavaLexBitSet();
                }
                 ...
                 * bunch.m_nfa_bit.size()); System.out.println("Reference index: " +
...
class CInput {
    ...
    boolean getLine() throws java.io.IOException {
        ...
        while (true) {
            ...
            /*
             * Old Source: m_line[m_line_read] = (char)
             * m_buffer[m_buffer_index]; ++m_line_read; ++m_buffer_index; New
             * Source:
             */
            if ((byte) '\r' == m_buffer[m_buffer_index]) {
                /*
                 * This is a hack and may propagate some (pre-existing) line
                 * counting bugs on non-UNIX, but otherwise it will work fine.
                 * The only effect will be some bad error messages. The other
                 * option is to do some massive hacking on the input routines
...
class CUtility {
    ...
    /***************************************************************************
     * Function: cuAssert Description: Debugging routine. Renamed from assert
     * because Java 1.4 uses assert as a keyword
     **************************************************************************/
...
class CLexGen {
    ...
    private char[] packCode(char start_dir[], char end_dir[], char prev_code[],
        ...
        if (CUtility.DEBUG) {
            CUtility.cuAssert(INIT_CODE == specified || CLASS_CODE == specified
    ...
    /***************************************************************************
     * Function: printccl Description: Debuggng routine that outputs readable
 ...
 *
 * Revision: 10/21/96 Modified by Elliot Joel Berk (ejberk@princeton.edu), for
 * the purposes of fixing a bug that was causing unjustified exceptions and
 * crashes. This bug was the result of resizing the internal buffer not large
 * enough in some cases. The name was then changed to JavaLexBitSet.
 *
