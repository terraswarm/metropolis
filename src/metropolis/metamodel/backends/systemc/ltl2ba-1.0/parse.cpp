...
{        Node *a, *b;
 ...
 if (ptr)
     switch (ptr->ntyp) {
     case U_OPER:
         if (ptr->rgt->ntyp == LTL_TRUE
                 ||  ptr->rgt->ntyp == LTL_FALSE
                 ||  ptr->lft->ntyp == LTL_FALSE)
             {        ptr = ptr->rgt;
             break;
             }
         ...
         if (ptr->lft->ntyp == LTL_TRUE &&
                 ptr->rgt->ntyp == V_OPER &&
                 ptr->rgt->lft->ntyp == LTL_FALSE &&
                 ptr->rgt->rgt->ntyp == U_OPER &&
                 ptr->rgt->rgt->lft->ntyp == LTL_TRUE) {
             ptr = ptr->rgt;
             break;
         }
         ...
         break;
     case V_OPER:
         if (ptr->rgt->ntyp == LTL_FALSE
                 ||  ptr->rgt->ntyp == LTL_TRUE
                 ||  ptr->lft->ntyp == LTL_TRUE)
             {        ptr = ptr->rgt;
             break;
             }
         ...
         if (ptr->lft->ntyp == LTL_FALSE &&
                 ptr->rgt->ntyp == U_OPER &&
                 ptr->rgt->lft->ntyp == LTL_TRUE &&
                 ptr->rgt->rgt->ntyp == V_OPER &&
                 ptr->rgt->rgt->lft->ntyp == LTL_FALSE) {
             ptr = ptr->rgt;
             break;
         }
     ...
     case NEXT:
         ...
         if (ptr->lft->ntyp == V_OPER &&
                 ptr->lft->lft->ntyp == LTL_FALSE &&
                 ptr->lft->rgt->ntyp == U_OPER &&
                 ptr->lft->rgt->lft->ntyp == LTL_TRUE) {
             ptr = ptr->lft;
             break;
         }
         /* NEW : X F G p == F G p */
         if (ptr->lft->ntyp == U_OPER &&
                 ptr->lft->lft->ntyp == LTL_TRUE &&
                 ptr->lft->rgt->ntyp == V_OPER &&
                 ptr->lft->rgt->lft->ntyp == LTL_FALSE) {
             ptr = ptr->lft;
             break;
         }
         break;
     ...
     case AND:
         ...
         if (isequal(ptr->lft, ptr->rgt)        /* (p && p) == p */
                 ||  ptr->rgt->ntyp == LTL_FALSE        /* (p && F) == F */
                 ||  ptr->lft->ntyp == LTL_TRUE        /* (T && p) == p */
                 ||  implies(ptr->rgt, ptr->lft))/* NEW */
             {        ptr = ptr->rgt;
             break;
             }
         if (ptr->rgt->ntyp == LTL_TRUE        /* (p && T) == p */
                 ||  ptr->lft->ntyp == LTL_FALSE        /* (F && p) == F */
     ...
     case OR:
         ...
         if (isequal(ptr->lft, ptr->rgt)        /* (p || p) == p */
                 ||  ptr->rgt->ntyp == LTL_FALSE        /* (p || F) == p */
                 ||  ptr->lft->ntyp == LTL_TRUE        /* (T || p) == T */
                 ||  implies(ptr->rgt, ptr->lft))/* NEW */
             {        ptr = ptr->lft;
             break;
             }
         if (ptr->rgt->ntyp == LTL_TRUE        /* (p || T) == T */
                 ||  ptr->lft->ntyp == LTL_FALSE        /* (F || p) == p */
                 ||  implies(ptr->lft, ptr->rgt))/* NEW */
             {        ptr = ptr->rgt;
             break;
             }
...
{        Node *ptr = ZN;
 ...
 case NEXT:
     ...
     if ((ptr->ntyp == LTL_TRUE || ptr->ntyp == LTL_FALSE)&& tl_simp_log)
