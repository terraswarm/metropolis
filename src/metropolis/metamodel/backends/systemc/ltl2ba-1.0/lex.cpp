...
{        int c;
 ...
 if (islower(c))
     {        getword(c, isalnum_);
     if (strcmp("true", ltl2ba_yytext) == 0)
         {        Token(LTL_TRUE);
         }
     if (strcmp("false", ltl2ba_yytext) == 0)
         {        Token(LTL_FALSE);
         }
     tl_yylval = tl_nn(PREDICATE,ZN,ZN);
     tl_yylval->sym = tl_lookup(ltl2ba_yytext);
     return PREDICATE;
     }
 if (c == '<')
     {        c = tl_Getchar();
