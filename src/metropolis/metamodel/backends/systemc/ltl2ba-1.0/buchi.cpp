...
BScc *bscc_stack;
int lt_accept, bstate_count = 0, btrans_count = 0, rank;
...
{
    BTrans *s, *t;
    if (((a->final == lt_accept) || (b->final == lt_accept)) &&
            (a->final + b->final != 2 * lt_accept) &&
...
int bdfs(BState *s) {
    ...
    for (t = s->trans->nxt; t != s->trans; t = t->nxt) {
        if (t->to->incoming == 0) {
            ...
            scc->theta = ltl2bamin(scc->theta, result);
        }
        else {
            for (c = bscc_stack->nxt; c != 0; c = c->nxt)
                if (c->bstate == t->to) {
                    scc->theta = ltl2bamin(scc->theta, c->rank);
                    break;
                }
        }
    }
...
{
    if ((fin != lt_accept) && in_set(set, final[fin + 1]))
...
void make_btrans(BState *s) /* creates all the transitions from a state */
{
    ...
    if (s->gstate->trans)
        for (t = s->gstate->trans->nxt; t != s->gstate->trans; t = t->nxt) {
            int fin = next_final(t->final, (s->final == lt_accept) ? 0 : s->final);
