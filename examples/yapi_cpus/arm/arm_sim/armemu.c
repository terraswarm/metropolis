/*  armemu.c -- Main instruction emulation:  ARM7 Instruction Emulator.
    ...
    GNU General Publi1c License for more details.
...
#endif
{
  ...
  /* added by TCM for C++ portability */
  int do_int = 0;
  ...
  do
    {
      ...
      if (TFLAG)
        {
          ...
          switch (ARMul_ThumbDecode (state, pc, instr, &new_word))
            {
            case t_undefined:
      ...
      if (((temp = TOPBITS (28)) == AL) && !do_trace)
          ...
          if (ok && (cp14r0 & ARMul_CP14_R0_ENABLE))
            {
              ...
              if (cp14r0 & ARMul_CP14_R0_CCD)
                ...
                {
                  ARMword cp14r1;
                  /* change by TCM for c++ portability */
                  /* int do_int = 0; */
                  do_int = 0;
          ...
          if ((pc | 3) == (read_cp15_reg (14, 0, 8) | 2)
              || (pc | 3) == (read_cp15_reg (14, 0, 9) | 2))
            {
              if (XScale_debug_moe (state, ARMul_CP14_R10_MOE_IB))
                {
                  printf ("XScale-debug->handleSWI1\n");
      ...
      /* Generates the instruction trace, exits upon multiple repeat inst's */
      /* Added by TCM */
      if (do_trace)
        {
          ...
          if (pc == prev_pc)
            {
              prev_inst_count++;
              if (prev_inst_count > REPEAT_INST_LIMIT)
                {
                  printf ("Repeat limit reached, exiting\n");
                  fprintf (trace_file, "NO\n");
                  fclose (trace_file);
                  exit (-1);
                }
            }
          ...
          // NOTE: ADD IN ADDRESSES FOR LOADS + STORES via GetLSRegRHS
          fprintf (trace_file, "EX %x %x\n", pc, instr);
          //fclose(trace_file);
        }
      ...
      if (temp)
        ...
        mainswitch:
          ...
          switch ((int) BITS (20, 27))
                  ...
                  else if (state->is_XScale)
                    {
                      ...
                      /* Only accumulator 0 is supported.  */
                      ...
                      else
                        {
                          ...
                          state->Accumulator +=
                            (ARMdword) state->Reg[BITS (16, 19)] << 32;
                        }
                    }
                  else
            ...
            case 0xfe:
            case 0xff:
              printf ("OSHandleSWI4--- swi instruction\n");
#ifndef SKIP_SWI
              ...
              if (!ARMul_OSHandleSWI (state, BITS (0, 23)))
                ARMul_Abort (state, ARMul_SWIV);
#else
              printf ("SKIP_SWI DEFINED, doing nothing...\n");
              ARMul_SetPC (state, ARMul_GetPC (state) + 4);
#endif
              break;
            }
        }
...
Handle_Load_Double (ARMul_State * state, ARMword instr)
{
  ...
  /* Compute the offset.  */
  offset =
    immediate ? ((BITS (8, 11) << 4) | BITS (0, 3)) : state->Reg[RHSReg];
